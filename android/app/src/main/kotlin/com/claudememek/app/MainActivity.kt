...
class MainActivity : FlutterActivity() {
    private val channel = "com.claudememek.app/native"
            ...
            .setMethodCallHandler { call, result ->
                try {
                    when (call.method) {
                        ...
                        // M22 — expose keychain CSV (native, XOR-obfuscated)
                        "getApiKeys" -> result.success(NativeBridge.getApiKeys())
                        "getDatasetKey" -> result.success(NativeBridge.getDatasetKey())
                        "sentinelProbe" -> result.success(NativeBridge.sentinelProbe())
...
/// dynamic linker JNI bisa resolve simbol `Java_com_claudememek_app_NativeBridge_*`.
object NativeBridge {
    init {
        ...
        System.loadLibrary("keychain") // M22
        System.loadLibrary("sentinel") // M33 runtime checks
        System.loadLibrary("memek")
        // Decoy is independent. A failed load must never brick real JNI.
        try {
            System.loadLibrary("idinganteng")
        } catch (_: UnsatisfiedLinkError) {
            // Keep the application operational on unusual package splitters.
        }
    }
    ...
    /// M22 — Return CSV daftar AI API key (hasil deobfuscation native).
    @JvmStatic external fun getApiKeys(): String
    /// M32 — dataset XOR key (single byte).
    @JvmStatic external fun getDatasetKey(): Int
    /// M33 — debugger/instrumentation/root/emulator detector bitmask.
    @JvmStatic external fun sentinelProbe(): Int
}
